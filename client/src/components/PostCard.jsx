...
const PostCard = ({ post }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (data.success) {
        toast.success(data.message);
        // Optionally, you can refresh the posts or remove the deleted post from the state
      } else {
        toast.error(data.message);
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl">
      ...
      <div className="flex items-center gap-4 text-gray-600 text-sm pt-2 border-t border-b-gray-300">
        ...
        {post.user._id === currentUser._id && (
          <div className="flex items-center gap-1">
            <Trash2 onClick={handleDelete} className="w-4 h-4 cursor-pointer text-red-500" />
          </div>
        )}
      </div>
    </div>
