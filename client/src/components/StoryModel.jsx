...
const StoryModel = ({ setShowModel, fetchStories }) => {
    ...
    const bgColors = [
        "#4f46e5", "#7c3aed", "#db2777", "#e11d48", "#ca8a04", "#0d9488"
    ]
        ...
        if (file) {
            if (file.type.startsWith("video")) {
                if (file.size > MAX_VIDEO_SIZE_MB * 1024 * 1024) {
                    toast.error("Video file size exceeds more than 50 MB")
                    setMedia(null)
                    setPreviewUrl(null)
                    return;
                }
                ...
                video.onloadedmetadata = () => {
                    ...
                    if (video.duration > MAX_VIDEO_DURATION) {
                        toast.error("Video Duration more than 1 minute")
                        setMedia(null)
                        setPreviewUrl(null)
        ...
        <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
            <div className='w-full max-w-md'>
                ...
                <div className='flex mt-4 gap-2'>
                    {bgColors.map((color) => (
                        <button onClick={() => setBackground(color)} style={{ backgroundColor: color }}
                ...
                <button className='flex items-center justify-center gap-2 text-white py-3 mt-4 w-full bg-gradient-to-r
